...
static void * malloc_func(int size)
{
  LOG("hook malloc %d",size);
   return malloc_prev(size);
}

static void* calloc_func(int count,int size)
{
   void* prev = calloc_prev(count,size);
//   LOG("calloc count:%d, size:%d",count,size);
   return prev;
}
...
static int hacker_hook(JNIEnv *env, jobject thiz) {
  ...
  calloc_stub = bytehook_hook_single("libhooksample.so",NULL,"calloc", (void*)calloc_func,calloc_hooked_callback,NULL);
...
//  void *calloc_proxy = (void *) calloc_func;
